...
/*   Updated: 2021/01/23 03:04:16 by Ahmed            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_putlstrx(t_f *t_flag, int dot)
{
	...
	i = 0;
	if (t_flag->type_x == 0 && !t_flag->dot && t_flag->type_p == 0)
	{
		if (t_flag->type_s)
			free(t_flag->type_s);
		return (0);
...
char	*ft_hexadecimal(uintmax_t x, unsigned int len_x)
{
	...
	i = 0;
	if (!(str = (char *)malloc(sizeof(char) * ((len_x) + 1))))
		return (0);
	ft_bzero(str, len_x + 1);
	ft_bzero(qwe, len_x + 1);
	while (1)
	{
		a = (x / 16) * 16;
